...
local C = {
    ...
    accent2  = {  92, 140, 255 },  -- secondary blue (used sparingly)
    panel    = {  11,  11,  16 },  -- panel base
    panelTop = {  16,  15,  24 },  -- subtle lighter
    row      = {  15,  15,  21 },
    rowAlt   = {  18,  18,  25 },
    line     = {  32,  30,  44 },  -- separators / borders
    text     = { 236, 236, 244 },
    textDim  = { 132, 128, 158 },
    textOnAccent = { 8, 8, 12 },
    track    = {  38,  36,  52 },
...
function Menu.DrawHeader()
    ...
    local pulse = Menu.Pulse(0.8, 150, 230)
    Menu.Rect(x, y + bh - 2 * s, w, 2 * s, C.accent, pulse, 0)
    ...
    if alpha > 0 then
        ...
        if alpha >= 1 then
            ...
            Menu.Text(tx + bw + 4 * s, ty + 4 * s, ".lua", 12, C.accent, 220)
        end
    end
end
...
function Menu.DrawSectionBar()
    ...
    -- accent tick on the right
    local tickW = 22 * s
    Menu.Rect(x + w - tickW - 16 * s, y + h / 2 - 1, tickW, 2, C.accent, 255, 0)
end
...
function Menu.DrawItem(x, itemY, w, ih, item, isSel)
    ...
    if item.badge and item.type ~= "toggle" and item.type ~= "slider" then
        ...
        local bgCol = isSel and {255,255,255} or C.accent
        Menu.Rect(bx, by, bw, bhh, bgCol, isSel and 60 or 36, 4)
        local btw = Menu.TextW(item.badge, 10)
        Menu.Text(bx + (bw - btw) / 2, by + bhh / 2 - 5 * s, item.badge, 10, isSel and C.textOnAccent or C.accent, 230)
    end
end
...
local NOTIFY_CFG = {
    success = { c = {64, 220, 120}, title = "SUCCESS" },
    info    = { c = {124, 92, 255}, title = "INFO"    },
    warning = { c = {245, 195, 60}, title = "WARNING" },
